...
function dzdt = combined_system(t, z, A, B, C, K, L)
    ...
    y = C * x;%expected value obtained from simulation
    yhat = C*xhat; %measured value

    % Noise parameters
    sigma = 0.1;  % Standard deviation of noise
    z = sigma * randn;  % Gaussian noise

    y_noise = yhat +z; %noise induced in the measurement
    ...
    % Observer dynamics
    dxhatdt = A*xhat + B*u + L*(y-y_noise);
